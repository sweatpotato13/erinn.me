...
if (!inputPath || !process.env.NXOPEN_API_URL || !process.env.NXOPEN_API_KEY) {
    console.error(
...
const parsedInput = candidateInputSchema.safeParse(readJson(inputPath));
...
async function hasEvidence(path: string, name: string): Promise<boolean> {
    const url = new URL(path, process.env.NXOPEN_API_URL);
