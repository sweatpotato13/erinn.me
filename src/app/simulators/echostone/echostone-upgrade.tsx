...
export default function EchostoneUpgrade({
    colors,
    data,
    value,
    onChange,
    ...
    const step = data.steps.find(s => s.grade === value.grade)!;
    const last = value.history.at(-1);
    const goal = Math.max(target, value.grade);
        ...
        try {
            ...
            do {
                for (let i = 0; i < (repeat ? 100 : 1); i++) {
                    if (
                        cancelled.current ||
                        next.grade >= (repeat ? goal : 30) ||
                        count >= 10000
                    )
                        break;
                    next = upgradeEcho(data, next);
                    count++;
                }
                onChange(next);
                if (
    ...
    return (
        <div className={styles.workspace}>
            <section
                ...
                aria-labelledby="echo-upgrade-title"
            >
                ...
                <div className={styles.result}>
                    ...
                    >
                        ...
                        {data.stats.map((stat, i) => (
                            <p key={stat} className={styles.abilityName}>
                                {stat} {value.stats[i]}
                                {value.grade < 30 && (
                ...
                <p className={styles.hint}>
                    1등급·고유 스탯 1에서 시작하며, 증가량은 표시된 범위에서
                    같은 확률로 결정합니다.
                </p>
            </section>
            <section
