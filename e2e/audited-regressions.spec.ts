...
test("auction result filters stay local and responsive", async ({ page }) => {
    const counts = await openMarket(page);
    await searchMarket(page);
    const listings = page.getByRole("region", { name: "현재 등록 매물" });
    ...
    await trigger.click();
    const dialog = page.getByRole("dialog", { name: "결과 필터" });
    ...
    await expect(trigger).toBeFocused();
    await trigger.click();
    await page.getByLabel("정확한 아이템").selectOption("공통 아이템");
    await page.getByLabel("최소 단가").fill("100");
    await page.getByLabel("최대 단가").fill("400");
    ...
    await page.getByLabel("최소 단가").fill("300");
    await page.getByRole("button", { name: "적용" }).click();
    await expect(
